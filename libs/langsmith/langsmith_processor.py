...
if DEBUG and not logger.handlers:
    ...
    handler.setFormatter(
        logging.Formatter("%(asctime)s [LANGSMITH] %(levelname)s %(message)s")
    )
...
class LangSmithSpanProcessor(SpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        print(
            f"[LANGSMITH-PROCESSOR] Processing span: {span.name}",
            file=sys.stderr,
            flush=True,
        )
        ...
        if (
            ...
            self._set_prompt_attributes(
                span, [{"role": "user", "content": "audio_segment"}]
            )
            if transcript:
        ...
        # TTS span: text -> audio
            ...
            "tts" in span_name
            or "text_to_speech" in span_name
            or "synthesis" in span_name
        ):
            span._attributes["langsmith.span.kind"] = "llm"
            ...
            print(
                f"\n[LANGSMITH-PROCESSOR] 🔊 TTS SPAN: {span.name}",
                file=sys.stderr,
                flush=True,
            )
            print(
            ...
            if tts_metrics:
                try:
                    ...
                    if isinstance(metrics_data, dict):
                        ...
                        model_name = metadata.get("model_name") or metrics_data.get(
                            "model_name"
                        )
                        if model_name:
            ...
            self._set_prompt_attributes(
                ...
                [
                    {
                        "role": "system",
                        "content": f"Convert to speech with voice: {voice_id}",
                    },
                    {
                        "role": "user",
                        "content": str(text) if text else "text_to_speech",
                    },
                ],
            )
            self._set_completion_attributes(
        ...
        # Agent/Chain/Job spans: aggregate conversation
            ...
            if conv_msgs:
                system_msg, first_user_msg, remaining_msgs = (
                    self._split_conversation_messages(conv_msgs)
                )
        ...
        # Default: mark as chain if no specific type detected
            ...
            if span.attributes.get("input") or span.attributes.get("output"):
                ...
                if input_val:
                    self._set_prompt_attributes(
                        span, [{"role": "user", "content": str(input_val)}]
                    )
                if output_val:
                    self._set_completion_attributes(
    ...
    def _fallback_messages(self, span: ReadableSpan, span_name: str) -> list:
        ...
        system_prompt = (
            span.attributes.get("gen_ai.system") or span.attributes.get("system") or ""
        )
        user_prompt = (
    ...
    def _extract_llm_messages(self, span: ReadableSpan) -> list:
        ...
        if chat_ctx:
            ...
            try:
                ...
                print(
                    f"    ✗ Strategy 1 FAILED: {type(e).__name__}: {e}",
                    file=sys.stderr,
                    flush=True,
                )
        else:
        ...
        print(
            f"  🔍 Strategy 2: Checking gen_ai.request.prompt.*...",
            file=sys.stderr,
            flush=True,
        )
        messages = []
        ...
        if messages:
            ...
            print(
                f"    ✗ No gen_ai.request.prompt.* attributes found",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(
            f"  🔍 Strategy 2b: Checking gen_ai.prompt.*...",
            file=sys.stderr,
            flush=True,
        )
        idx = 0
        ...
        if messages:
            ...
            print(
                f"    ✗ No gen_ai.prompt.* attributes found",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(
            f"  🔍 Strategy 3: Checking messages/llm.messages/input attributes...",
            file=sys.stderr,
            flush=True,
        )
        messages_attr = (
        ...
        if messages_attr:
            try:
                if isinstance(messages_attr, str):
                    if DEBUG:
                        logger.debug(
                            f"  Parsing JSON string, length={len(messages_attr)}"
                        )
                ...
                print(
                    f"    ✗ Strategy 3 FAILED: {type(e).__name__}: {e}",
                    file=sys.stderr,
                    flush=True,
                )
        else:
        ...
        print(
            f"  🔍 Strategy 4: Checking individual system/user/assistant attributes...",
            file=sys.stderr,
            flush=True,
        )
        system = (
        ...
        assistant = span.attributes.get("gen_ai.assistant") or span.attributes.get(
            "assistant"
        )
        ...
        print(
            f"  ⚠️  All strategies failed - no messages extracted",
            file=sys.stderr,
            flush=True,
        )
        return []
    ...
    def _extract_llm_output(self, span: ReadableSpan) -> str:
        ...
        print(
            f"    Strategy 1: Checking lk.response.text...", file=sys.stderr, flush=True
        )
        ...
        print(
            f"    Strategy 2: Checking gen_ai.response.text / gen_ai.completion.text...",
            file=sys.stderr,
            flush=True,
        )
        ...
        if output:
            ...
            print(
                f"      ✗ gen_ai.response.text and gen_ai.completion.text not found",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(
            f"    Strategy 3: Checking common attribute names...",
            file=sys.stderr,
            flush=True,
        )
        output = (
        ...
        if output:
            ...
            print(
                f"      ✗ No common output attributes found",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(
            f"    Strategy 4: Checking gen_ai.completion.* attributes...",
            file=sys.stderr,
            flush=True,
        )
        idx = 0
        ...
        if completion_parts:
            ...
            print(
                f"      ✗ No gen_ai.completion.* attributes found",
                file=sys.stderr,
                flush=True,
            )

        print(
            f"    ⚠️  All strategies failed - no output extracted",
            file=sys.stderr,
            flush=True,
        )
        return ""
    ...
    def _track_messages(
        self, target_dict: dict, key: str, messages: list, output_data: str
    ):
        """
    ...
    def _defer_job_span(self, trace_id: str, span: ReadableSpan):
        ...
        print(
            f"⏸️  Deferring export of job span for trace {trace_id}",
            file=sys.stderr,
            flush=True,
        )

    def _release_job_span_if_waiting(
        self, trace_id: str, prompt_msgs: list, completion_msgs: list
    ):
        ...
        print(
            f"🧩 Releasing deferred job span for trace {trace_id}",
            file=sys.stderr,
            flush=True,
        )
        if prompt_msgs:
